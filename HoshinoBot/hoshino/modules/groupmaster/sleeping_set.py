...
async def sleep_8h(bot, ev):
    await bot.send('哦呀斯密大小姐')
    await util.silence(ev, 8 * 60 * 60, skip_su=False)


@sv.on_fullmatch(('奶龙套餐', '奶龙可爱', '来一份精致奶龙套餐', '我爱奶龙'))
async def sleep_forever(bot, ev):
    await bot.send('对')
    await util.silence(ev, 8 * 60 * 60 * 30, skip_su=False)
