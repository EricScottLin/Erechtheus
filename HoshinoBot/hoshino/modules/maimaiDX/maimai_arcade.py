...
<店名/别名>人数设置,设定,=,有,增加,加,来了,+,减少,减,走了,-, <人数> 操作排卡人数
<店名/别名> 增加,加,+,++,减少,减,-,-- 直接操作排卡人数
<店名/别名>有多少人,有几人,有几卡,几人,几卡,j,jk,jr 查看排卡人数
机厅几人,j,jk 查看已订阅机厅排卡人数"""

SV_HELP = '请使用 帮助maimaiDX排卡 查看帮助'
sv_arcade = Service('maimaiDX排卡', manage_priv=priv.ADMIN, enable_on_default=True, help_=SV_HELP)
...
async def add_arcade(bot: NoneBot, ev: CQEvent):
    ...
    if not priv.check_priv(ev, priv.SUPERUSER):
        ...
        msg = '添加机厅指令格式：添加机厅 <店名> <位置> [别称1] [别称2] ...'
    elif len(args) >= 3:
        if not args[2].isdigit():
            msg = '格式错误：添加机厅 <店名> <地址> [别称1] [别称2] ...'
        else:
...
@sv_arcade.on_prefix(['添加机台'])
async def add_arcades_arcade(bot: NoneBot, ev: CQEvent):
    args: List[str] = ev.message.extract_plain_text().strip().split()
    if not priv.check_priv(ev, priv.SUPERUSER):
        msg = '仅允许主人添加机台\n请使用 来杯咖啡+内容 联系主人'
    elif len(args) == 1 and args[0] in ['帮助', 'help', '指令帮助']:
        msg = '添加机台指令格式：添加机台 <店名> <机台名> <机台数量> <机台1别称> <机台2别称> ... <机台n别称>...'
    elif len(args) >= 3:
        if not args[2].isdigit():
            msg = '格式错误：添加机台 <店名> <机台名> <机台数量> <机台1别称> <机台2别称> ... <机台n别称>...'
        else:
...
@sv_arcade.on_rex(
    r'^(.+)?\s?(设置|设定|＝|=|有|增加|添加|加|来了|＋|\+|减少|降低|减|走了|－|-)\s?([0-9]+|＋|\+|－|-)(人|卡)?$')
...
@sv_arcade.on_rex(r'^(.+)?\s?(增|＋＋|\+\+|少|－－|--)$')
async def arcade_card(bot: NoneBot, ev: CQEvent):
    try:
        match: Match[str] = ev['match']
        gid = ev.group_id
        nickname = ev.sender['nickname']
...
@sv_arcade.on_suffix(['有多少人', '有几人', '有几卡', '多少人', '多少卡', '几人', 'jr', '几卡', 'j', 'jk'])
