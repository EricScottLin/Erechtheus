        ...
        async with aiohttp.ClientSession() as session:
            async with session.request(method, url, **kwargs) as response:
                if response.status == 200:
                    try:
                        return await asyncio.wait_for(response.json(), timeout=59)
...
class MaimaiAPI:
    ...
    async def music_data(self):
        """获取曲目数据"""
        return await _request('GET', self.MaiAPI + '/music_data')

    async def chart_stats(self):
        """获取单曲数据"""
        return await _request('GET', self.MaiAPI + '/chart_stats')
    ...
    async def query_user(self, project: str, *, qqid: Optional[int] = None, username: Optional[str] = None,
        ...
        if project == 'player':
            json['b50'] = True
        return await _request('POST', self.MaiAPI + f'/query/{project}', json=json)
    ...
    async def query_user_dev(self, *, qqid: Optional[int] = None, username: Optional[str] = None):
        ...
        return await _request('GET', self.MaiAPI + f'/dev/player/records', headers=self.headers, params=params)

    async def query_user_dev2(self, *, qqid: Optional[int] = None, username: Optional[str] = None,
                              music_id: Union[str, List[Union[int, str]]]):
        """
        ...
        return await _request('POST', self.MaiAPI + f'/dev/player/record', headers=self.headers, json=json)

    async def rating_ranking(self):
        """获取查分器排行榜"""
        return await _request('GET', self.MaiAPI + f'/rating_ranking')

    async def get_alias(self):
        """获取所有别名"""
        return await _request('GET', self.MaiAliasAPI + '/maimaidxalias')

    async def get_songs(self, name: str):
        """使用别名查询曲目"""
        return await _request('GET', self.MaiAliasAPI + '/getsongs', params={'name': name})

    async def get_songs_alias(self, song_id: int):
        """使用曲目 `id` 查询别名"""
        return await _request('GET', self.MaiAliasAPI + '/getsongsalias', params={'song_id': song_id})

    async def get_alias_status(self):
        """获取当前正在进行的别名投票"""
        return await _request('GET', self.MaiAliasAPI + '/getaliasstatus')

    async def get_alias_end(self):
        """获取五分钟内结束的别名投票"""
        return await _request('GET', self.MaiAliasAPI + '/getaliasend')

    async def transfer_music(self):
        """中转查分器曲目数据"""
        return await _request('GET', self.MaiAliasAPI + '/maimaidxmusic')

    async def transfer_chart(self):
        """中转查分器单曲数据"""
        return await _request('GET', self.MaiAliasAPI + '/maimaidxchartstats')
    ...
    async def post_alias(self, id: int, aliasname: str, user_id: int):
        ...
        return await _request('POST', self.MaiAliasAPI + '/applyalias', json=json)
    ...
    async def post_agree_user(self, tag: str, user_id: int):
        ...
        return await _request('POST', self.MaiAliasAPI + '/agreeuser', json=json)
            ...
            if 1000 < song_id < 10000 or 10000 < song_id <= 11000:
                for _id in [song_id + 10000, song_id - 10000]:
                    if (file := coverdir / f'{_id}.png').exists():
                        return file
            pic = await _request('GET', self.MaiCover + f'/{song_id:05d}.png')
            return BytesIO(pic)
        except CoverError:
    ...
    async def qqlogo(self, qqid: int) -> bytes:
        ...
        return await _request('GET', self.QQAPI, params=params)
